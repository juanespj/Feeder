...
static void
update_gatt_db_notif (cy_stc_ble_gatt_write_param_t *write_param);
static void
update_gatt_db_write (cy_stc_ble_gatt_write_param_t *write_param);
void
ble_stack_event_handler (uint32_t event, void *eventParam);
static void
ble_init (void);
static void
bless_interrupt_handler (void);
static void
ble_start_advertisement (void);
static void
ble_ias_callback (uint32 event, void *eventParam);
/* Variable used to store the return values of BLE APIs */
cy_en_ble_api_result_t ble_api_result;
uint32_t ledtimer = 0;

void
ble_feeder_init (void)
{
  /* Configure BLE */
  ble_init ();
  /* Init RTC */
...
void
ble_stack_event_handler (uint32 event, void *eventParam)
{
  ...
  switch (event)
    ...
    case CY_BLE_EVT_GATTS_XCNHG_MTU_REQ:
      {
	uartprint("[CY_BLE_EVT] GATT MTU Exchange Request received \r\n");
	cy_stc_ble_gatt_xchg_mtu_param_t mtu =
	  { .connHandle =
	      ((cy_stc_ble_gatt_xchg_mtu_param_t*) eventParam)->connHandle };
	Cy_BLE_GATT_GetMtuSize (&mtu);
	char msg[40];
	sprintf (msg, "GATTS_XCNHG_MTU_REQ %x, %x,", mtu.connHandle.attId,
		 mtu.connHandle.bdHandle);
	uartprint(msg);
	sprintf (msg, "final mtu= %d \r\n", mtu.mtu);
	uartprint(msg);
	break;
      }
    ...
    case CY_BLE_EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ:
      {
	uartprint("EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ");
	char msg[40];
	sprintf (
	    msg,
	    " %x %x: handle: %x \r\n",
...
//	uartprint("\r\n");
    ...
    case CY_BLE_EVT_GATTS_INDICATION_DISABLED:
      uartprint("CY_BLE_EVT_GATTS_INDICATION_DISABLED \r\n");
      break;

    case CY_BLE_EVT_GATTS_INDICATION_ENABLED:
      uartprint("CY_BLE_EVT_GATTS_INDICATION_ENABLED \r\n");
      break;
    default:
      {
	// printf("[INFO] : BLE Event 0x%lX\r\n", (unsigned long) event);
	char msg[40];
	sprintf (msg, "[INFO] : BLE Event other 0x%02X", event);
	uartprint(msg);
...
update_gatt_db_notif (cy_stc_ble_gatt_write_param_t *write_param)
{
  ...
  gattErrorCode = Cy_BLE_GATTS_WriteAttributeValuePeer (
      &conn_handle, &(write_param->handleValPair));
  if (gattErrorCode == CY_BLE_GATT_ERR_NONE)
    {
	  ...
	  == CY_BLE_TRIGGER_MANUALTRIG_CLIENT_CHARACTERISTIC_CONFIGURATION_DESC_HANDLE)
	{
	  /* If notifications are enabled */
	  if (write_param->handleValPair.value.val[0])
	    {
	      /* Custom data value for BLE notification. */
	      uint8_t custom_notif_data[CY_BLE_GATT_MTU] =
		{ 0 };
